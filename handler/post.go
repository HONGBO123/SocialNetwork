...
func uploadHandler(w http.ResponseWriter, r *http.Request) {
    ...
    // 后端的后缀，通过header.Filename. return the file suffix.
    suffix := filepath.Ext(header.Filename)
    if t, ok := mediaTypes[suffix]; ok { // key containes Java,  go  is  ok true check key in the value
        p.Type = t // video and image 
    } else {
        p.Type = "unknown"
    }
...
// *var req *Request vs var req Request
// req.header = "Richard"
